  ...
  public function login() {
    if (isset($_POST['btnSubmit'])) { // POST
      ...
      if ($cust != null && $cust->active == 1) {
        ...
        MyUtil::showAlertAndRedirect('Please re-enter', '?controller=customer&action=login');
      }
    } else { // GET
      $this->render('login');
    }
    ...
    if (isset($_POST['btnSubmit'])) { // POST
      if (isset($_SESSION['customer'])) {
        ...
        if ($result) {
          ...
          MyUtil::showAlertAndRedirect('SUCCESSFUL', '?controller=customer&action=home');
        }
      }
      MyUtil::showAlertAndRedirect('FAILURE', '?controller=customer&action=myprofile');
    } else { // GET
